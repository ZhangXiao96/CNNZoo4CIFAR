...
def create_jacobian_tensor(y_tensor, x_tensor):
    """
    create Jacobian tensor.
    :param y_tensor: [batch, output_shape]
    :param x_tensor: [batch, sample_shape[:]]
    :return: jacobian_tensor: [batch, sample_shape[:]]
    """
    list_derivatives = []
    nb_classes = y_tensor.shape[-1]
    sample_shape = len(x_tensor.shape)-1
    for class_ind in range(nb_classes):
        derivatives, = K.gradients(y_tensor[:, class_ind], x_tensor)
        list_derivatives.append(derivatives)
    grads = K.stack(list_derivatives)
    jacobian_tensor = K.permute_dimensions(grads, [1, 0] + list(range(2, sample_shape+2)))
